...
import type { FordefiAuthenticationParams, FordefiUserInfo } from "./types.js";
    ...
    >
{
  ...
  authenticate = async (): Promise<FordefiUserInfo> => {
  ...
  getAuthDetails = async (): Promise<FordefiUserInfo> => {
    if (!this.signer) throw new Error("Not authenticated");

    return {
      addresses: await this.inner.request({
        method: "eth_accounts",
      }),
    };
  };
